...
public:
    ...
    int GetOutputMultiplier(int factory_id) const;    // output quantity = recipe base × level
...
- **产量加成**：工厂等级即单次产量倍率（Lv.1=x1, Lv.2=x2, ...）。产出数量=配方基础产出×工厂等级，不影响生产耗时、材料消耗和队列容量。
- `GetSpeedMultiplier` 已废弃，始终返回 1.0f；使用 `GetOutputMultiplier` 获取产量倍率。
